...
This repository is a **hands-on lab for Claude Code hooks**. The e-commerce query library is just a realistic codebase to work in — the real learning is in the `hooks/` directory and `.claude/settings.example.json`. By the end you'll have hooks that **block** dangerous actions, **format** and **type-check** edits automatically, **log** every tool call, and even **review** new code with a second Claude instance.

### Prerequisites

- **Node.js 20+** and npm
- **[Claude Code](https://claude.com/claude-code)** installed and authenticated
- An **Anthropic API key** (only needed for the optional query-duplication exercise, which uses the Agent SDK)
- **`jq`** on your `PATH` (used by the logging and formatting hooks — `brew install jq` on macOS)

### Setup

```bash
git clone https://github.com/smakubi/claude-hooks-workshop.git
cd claude-hooks-workshop
npm run setup      # installs deps + generates .claude/settings.local.json from the $PWD template
```

`npm run setup` runs `scripts/init-claude.js`, which copies `.claude/settings.example.json` and replaces the `$PWD` placeholders with the absolute path to _your_ clone. This is why hook commands can use absolute paths (a Claude Code security recommendation) while the template stays shareable. `settings.local.json` is git-ignored, so every learner generates their own.
...
| ------------------------------------ | ----------------------- | ------------------------------------------------------------------------------------------------------ |
| PreToolUse `Write\|Edit\|MultiEdit`  | `hooks/query_hook.js`   | Reviews new query functions for duplicates using a second Claude (Agent SDK). **Disabled by default.** |
| PreToolUse `*`                       | `jq . > pre-log.json`   | Logs the full input of every tool call                                                                 |
| PostToolUse `Write\|Edit\|MultiEdit` | `jq … prettier --write` | Auto-formats the file Claude just edited                                                               |
| PostToolUse `Write\|Edit\|MultiEdit` | `hooks/tsc.js`          | Runs the TypeScript compiler; `exit 2` feeds any type errors back to Claude to fix                     |
| PostToolUse `*`                      | `jq . > post-log.json`  | Logs the full result of every tool call                                                                |

> `hooks/read_hook.js` (a PreToolUse blocker for the environment file) ships in the repo but is **not wired in** — that's your first exercise.

### Exercises

**Exercise 1 — Block reads of the secrets file.**
`hooks/read_hook.js` already blocks any `Read` whose path contains the environment filename (it exits `2` with a message). Wire it in: add a `PreToolUse` block matching `Read` to `.claude/settings.example.json`, re-run `npm run setup`, then ask Claude to read your environment file. It should be denied. Ask it to read `README.md` and it works normally.

**Exercise 2 — Catch duplicate queries.**
`hooks/query_hook.js` is short-circuited by a `process.exit(0)` on the first line of `main()`. Remove that line to enable it. Now work the brief in [`task.md`](./task.md) — it asks you to add a query for orders pending more than 3 days. The repo already has a `getPendingOrders()` function, so the hook should launch a reviewer Claude, notice the duplicate, and block the edit with feedback. (This costs extra tokens per query-file edit and needs your API key.)

**Observe the hooks working.** After any edit, inspect `pre-log.json` and `post-log.json` to see the exact JSON each hook received — the easiest way to learn each tool's input shape.

### Solution

A completed version of both exercises lives on the **`solution`** branch:
...
> **Critical convention:** _all database queries must live in `src/queries/`._ This is enforced/encouraged by the `hooks/query_hook.js` Claude Code hook and stated in `CLAUDE.md`.
...
This is a workshop starter repository. Fork or clone it freely for your own learning.
